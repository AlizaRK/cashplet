...
const AccountsView: React.FC<{
  ...
  const {
    ...
    addAccount: (accountData: AccountData) => void;
    editAccount: (id: string, accountData: AccountData) => void;
    ...
    <div className="space-y-6 animate-in slide-in-from-right duration-300">
      <header className="flex items-center justify-between">
        <button
          ...
          className="p-3 bg-white rounded-2xl border border-amber-50 text-amber-600 transition-all active:scale-90"
        >
          <ChevronLeft size={24} />
        </button>
      ...
      <div className="grid grid-cols-1 gap-4 pb-10">
        {accounts.map((acc) => {
          const config = getAccountConfig(
            acc.type.toUpperCase() as keyof typeof AccountTypes
          );
          ...
          return (
            <div
              key={acc.id}
              className="bg-white p-5 rounded-[2.2rem] flex items-center justify-between border border-amber-50 shadow-sm hover:shadow-md transition-shadow"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${config.theme}`}
                    >
                      {config.label}
                    </span>
                  </div>
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="p-3 text-gray-300 hover:text-amber-500 transition-colors"
                >
                  <Edit2 size={18} />
                </button>
                <button
